...
class MDP_SEQ(Abstract_MDP):
    ...
    def getQoS(self, state, action, time_step):
        """
        ...
        #    print 'QoS list for action', act, ' = ',  self.t.get_qos(act, str(time_step))
        if str_action in actions_list:
            return (action, self.t.get_qos(str_action, str(time_step)))
